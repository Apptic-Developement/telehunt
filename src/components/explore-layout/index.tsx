                ...
                >

                    {
                        filterButtons && filterButtons.map((button) => {
                            return (
                                <button onClick={() => (setActiveFilter(button.href))} className='hover:text-foreground transition-all duration-300 ease-in-out' key={button.href} id={activeFilter === button.href ? 'active-explore-filter-button' : ''}>{button.name}</button>
                            )
                        })
                    }
                </div>
